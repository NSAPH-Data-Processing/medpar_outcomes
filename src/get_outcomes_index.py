...
def main(args):
    ...
    for outcome in outcomes: #TODO replace with snakemake
        LOGGER.info(f"Preparing {outcome}----")

        #conn = duckdb.connect(f"duckpond_{outcome}.db")
        conn = duckdb.connect(f"duckpond.db")
        ...
        for year in years['year']:
            ...
            conn.execute(f""" COPY (
                ...
                WHERE o.year = {year}
            ) TO '{args.output_prefix}/{outcome}_{year}.parquet'
            """)
            LOGGER.info(f"# saved outcomes file at {args.output_prefix}/{outcome}_{year}.parquet ----")
